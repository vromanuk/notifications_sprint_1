...
class Command(BaseCommand):
    def handle(self, *args, **options):
        try:
            while True:
                ...
                if (
                    not Email.objects.filter(
                        status=Email.STATUS_CHOICES.queued.value[0]
                    )
    ...
    def _get_queued(self, batch_size=100):
        return (
            Email.objects.filter(status=Email.STATUS_CHOICES.queued.value[0])
    ...
    def _send_bulk(self, emails):
        ...
        Email.objects.filter(id__in=email_ids).update(
            status=Email.STATUS_CHOICES.failed.value[0]
        )
